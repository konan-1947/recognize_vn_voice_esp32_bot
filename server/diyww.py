...
def send_led_command(command):
    ...
    try:
        ...
        cmd_socket.sendto(message, (esp32_address[0], COMMAND_PORT))
        cmd_socket.close()
        
        print(f"✅ Đã gửi lệnh '{command}' đến ESP32 ({esp32_address[0]}:{COMMAND_PORT})")
        return True
...
def check_wake_word(text):
    ...
    if wake_word_lower in text_lower:
        print(f"🎯 WAKE WORD DETECTED: '{text}' chứa '{WAKE_WORD}'")
        return True
...
def udp_listener():
    ...
    while not shutdown_event.is_set():
        try:
            ...
            if esp32_address is None or esp32_address[0] != addr[0]:
                esp32_address = addr
